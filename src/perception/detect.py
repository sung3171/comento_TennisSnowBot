...
# autonavsim2d로 그린 map을 통해 라이다 좌표 정보를 뽑아서 그 좌표 배열을 다시 코드에 


# 테니스 코트장
# 테니스 코트장 좌표를 생성 = 라이다로 받은좌표라고 본다.
# autonavsim2d 에서 free space로 인식하는 색상은 WHITE, BLACK, GREEN, BLUE, RED, GREY, ORANGE 6가지입니다.
# 라이다 좌표 생성(현실에선 라이다로부터 받은좌표) 및 반환, 군집화 및 군집화 목적지(여기선 중앙값) 반환 그리고 좌표 print 등 함수를 보기좋게 독립적으로 해야한다.
